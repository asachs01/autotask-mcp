...
Implement a three-tool meta-interface inspired by mcp-dynamic-proxy:
...
1. Add tool categorization metadata to existing tools
...
6. Benchmark token usage before/after

**Test Strategy:**

1. Measure baseline token usage with all 35 tools loaded. 2. Implement lazy loading and measure reduced token usage. 3. Test tool discovery flow: list_categories -> list_tools -> execute_tool. 4. Verify backward compatibility - existing direct tool calls still work. 5. Test with Claude Code MCP Tool Search enabled. 6. Performance benchmark: response time for tool discovery vs direct call.
